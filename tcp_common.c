    ...
    while (!terminate_loop) {
        ...
        for (int i = 0; i < nr_fds; i++) {
            ...
            if (fds[i].revents != POLLIN) {
                LOG_ERROR("Got invalid event on fd = %d", fds[i].fd);
                terminate_loop = 1;
                break;
            }
            if (fds[i].fd == tun_fd) {
                ...
                if (nr_read <= 0) {
                    LOG_ERROR("Failed reading from tunnel %d : %s", fds[i].fd, strerror(errno));
                    terminate_loop = 1;
                    break;
                } else {
                    ...
                    if (nr_written <= 0) {
                        LOG_ERROR("Failed writing to socket %d :%s ", fds[i].fd, strerror(errno));
                        terminate_loop = 1;
                        break;
                    }
                ...
                LOG_DEBUG("nr_fds = %d, fd = %d", nr_fds, fds[i].fd);
                nr_read = read_nr_bytes(fds[i].fd, (char *) &packet_len, sizeof(packet_len));
                if (nr_read <= 0) {
                    LOG_ERROR("Failed reading socket %d :%s", fds[i].fd, strerror(errno));
                    close_connection = 1;
                } else {
                ...
                if (close_connection) {
                    LOG_WARN("Closing socket %d", fds[i].fd);
                    close(fds[i].fd);
                    fds[i].fd = -1;
                    break;
                }
